...
router.get('/:postId', async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute(
      `SELECT p.post_id, p.content, p.created_at, p.updated_at, u.user_id AS author_id, u.name AS author
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!user_id || !content)
      return res
        .status(400)
        .json({ ok: false, error: 'user_id and content required' });
...
router.delete('/:postId', async (req, res) => {
  try {
    const { postId } = req.params;
    const [result] = await pool.execute(`DELETE FROM Post WHERE post_id=?`, [
      postId,
    ]);
    res.json({ ok: true, data: { affectedRows: result.affectedRows } });
  } catch (err) {
...
/** 팔로우 기반 피드: followerId가 팔로우하는 사람들의 글(최신순) */
